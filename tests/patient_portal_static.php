...
$expect(str_contains($source['routes'], "Router::get('/identificar', 'PatientPortalController@home')") && str_contains($source['routes'], "Router::post('/identificar', 'PatientPortalController@identify')"), 'Links GET antigos de identificação não possuem compatibilidade ou enfraqueceram o POST seguro.');
...
$expect(str_contains($source['reports'], 'releasedReportByToken') && str_contains($source['reports'], 'portalPdfRequest') && str_contains($source['reports'], 'portalPdfTenantId') && str_contains($source['reports'], "'portalPatientPdf' => \$portalPatientPdf"), 'Renderer existente de PDF não valida escopo de paciente e tenant antes de servir o laudo.');
