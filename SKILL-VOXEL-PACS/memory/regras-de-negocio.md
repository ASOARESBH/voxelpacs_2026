...
| **Impersonação de Negócio (superadmin) é escopo de sessão, nunca permanente**: `POST /platform/negocios/{id}/impersonate` seta `$_SESSION['tenant_id']`/`impersonating_tenant_id`, some no logout, e é reversível a qualquer momento via `GET /platform/impersonate/exit`. Enquanto ativa, TODA tela tenant-scoped (Worklist, Médicos, Unidades, Configurações etc. — ver `architecture/auth-e-permissoes.md`) deve mostrar só o Negócio impersonado, nunca cross-tenant. Banner "Visualizando como: X" + botão "Sair da Impersonação" é **obrigatório** em qualquer layout que sirva tela tenant-scoped (hoje: `pacs_header.php`, o layout ativo). Início e fim da impersonação são sempre auditados (`AuditLogger::log('impersonate'/'exit_impersonate', ...)`) | `TenantsController::impersonate()/exitImpersonate()`, `TenantMiddleware`, `pacs_header.php` | Superadmin navega "invisível" (sem saber que está com visão restrita) ou, pior, o impersonate não propaga e ele vê/mistura dado de outro tenant sem perceber — quebra de confiança na fronteira multi-tenant |
