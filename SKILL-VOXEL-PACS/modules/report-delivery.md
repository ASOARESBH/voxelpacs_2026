...
2026-09-22 — Quando a versão liberada não possui corpo/seções versionadas suficientes, uma substituição operacional autorizada pode usar `ReportVersionPdfRevisionService::createOperationalReplacementFromCurrentReport()` sem criar snapshot canônico da versão. A revisão registra `source_kind=current_report_body`, usa o hash do PDF operacional como referência técnica de origem, preserva `report_versions` e falha fechado se o report não estiver liberado. Esse caminho não autoriza automaticamente Delivery Request, Outbox, Job ou transmissão.
