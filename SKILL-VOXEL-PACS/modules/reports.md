...
`ReportsController::pdfByToken()` resolve o token opaco e delega a renderização a `ReportsController::pdf()`. Laudos `assinado`/`liberado` com revisão operacional íntegra são servidos primeiro pela revisão mais recente da mesma `tenant_id`/report/version através de `ReportVersionPdfRevisionService::readLatestForViewer()`; na ausência de revisão, o viewer usa o snapshot binário canônico via `ReportVersionPdfSnapshotService`. O fluxo de artefato Non-DICOM usa `ReportDeliveryArtifactService` e permanece **fail-closed** quando a versão do job não possui snapshot válido.

Para laudos históricos assinados/liberados que não têm nenhum metadado de snapshot, a visualização interativa usa fallback explícito do renderer legado, sem alterar a versão clínica. O valor isolado `pdf_snapshot_schema_version=1`, introduzido por migration aditiva em linhas antigas, não constitui metadado de snapshot e também permite esse fallback. Metadado parcial real, caminho ausente acompanhado de path/hash/size/renderer, hash inválido ou arquivo corrompido não recebe fallback silencioso e continua sendo erro operacional auditável. O layout `moderno_lateral` usa tabelas de geometria fixa e quebra segura de valores para que a coluna de identificação do exame permaneça dentro da largura útil A4 do dompdf. No modo `print`, cabeçalho, identificação do paciente e conteúdo clínico devem permanecer limitados à largura útil de 174 mm do A4 com padding de 18 mm, e o container não deve impor `min-height: 297mm`, pois isso cria um fólio vazio no Dompdf.

O link público `/reports/r/{token}/pdf` resolve primeiro o token no escopo do portal e encaminha o `tenant_id` obtido da sessão, nunca de GET. O viewer autenticado e o renderer de snapshot aplicam o mesmo resolver de assets: logo da unidade somente em `uploads/unidades/`, incorporado como data URI a partir do `public/` ou do `STORAGE_PATH` atual. A consulta do PDF exige `reports.tenant_id` e `bi_pacs_estudos.tenant_id` iguais, evitando que o link selecione template/logo de outro tenant.
