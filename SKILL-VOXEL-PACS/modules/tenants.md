...
Não existe, em nenhum lugar do código, um vínculo médico → unidade → institution_name que restrinja o acesso de um médico específico a só algumas Unidades/InstitutionNames **dentro** de um mesmo tenant. Buscas por `medico_unidade`, `unidade_medico`, associação médico↔institution_name não encontraram nada (confirmado 2026-07-15). Se uma tarefa futura mencionar isso como "já implementado em fase anterior", é referência a planejamento de outra conversa, nunca construído — não presumir a existência, verificar no código primeiro.
