...
| Report Delivery Hub — `retryManualHomologationJob()` | `pacs_report_delivery_jobs`, `pacs_report_delivery_outbox`, `pacs_report_delivery_destinations`, `pacs_report_delivery_artifacts`, worker e `AuditLogger` | `ReportDeliveryController`, view `platform/negocios/report_delivery.php`, `bin/report_delivery_worker.php` | **Alto** — reativa somente um job terminal Non-DICOM de homologação após lock e validação de artifact; qualquer alteração de filtro tenant/destino/ambiente pode permitir retransmissão indevida ou atingir produção. O retry automático `retryJob()` permanece separado. |
