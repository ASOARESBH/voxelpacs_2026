    ...
    public function streamCsvMedicos(array $linhas, array $porMedico, array $totalizadores, array $resumoFiltros, string $tenantNome, string $usuarioNome, string $filename): void
    {
        ...
        foreach ($linhas as $linha) {
            fputcsv($out, [
                ...
                $linha['unidade'] ?? '', $linha['modalities'] ?? '', RelatorioProdutividadeMedicosRepository::prioridadeLabel((string) ($linha['prioridade'] ?? 'ROUTINE')),
                RelatorioProdutividadeMedicosRepository::prioridadeLabel((string) ($linha['prioridade_origem'] ?? 'ROUTINE')),
                !empty($linha['prioridade_manual']) ? 'SIM' : 'NÃO', $linha['medico_nome'] ?? '',
