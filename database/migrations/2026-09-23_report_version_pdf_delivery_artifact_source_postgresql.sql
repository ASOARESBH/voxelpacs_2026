-- VOXEL PACS — Proveniência de revisão PDF recuperada de artifact entregue (PostgreSQL)
-- Aditiva, tenant-scoped e sem alterar report_versions ou artifacts.
-- O artifact permanece histórico; esta migration apenas registra sua referência
-- explícita quando hash, tamanho, storage e vínculo entregue forem validados.
...
ALTER TABLE pacs_report_version_pdf_revisions
    ADD CONSTRAINT ck_report_pdf_revision_source_kind CHECK (
        ...
        (source_kind = 'delivery_artifact'
            ...
            AND source_delivery_artifact_sha256 ~ '^[0-9a-f]{64}$'
            AND source_delivery_artifact_size_bytes >= 100)
    );

ALTER TABLE pacs_report_version_pdf_revisions
    DROP CONSTRAINT IF EXISTS ck_report_pdf_revision_reason;
...
CREATE INDEX IF NOT EXISTS idx_report_pdf_revision_delivery_artifact
...
-- DROP INDEX IF EXISTS idx_report_pdf_revision_delivery_artifact;
-- ALTER TABLE pacs_report_version_pdf_revisions DROP CONSTRAINT ck_report_pdf_revision_reason;
-- ALTER TABLE pacs_report_version_pdf_revisions
--     ADD CONSTRAINT ck_report_pdf_revision_reason CHECK (
