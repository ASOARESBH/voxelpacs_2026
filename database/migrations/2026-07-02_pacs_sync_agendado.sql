...
ALTER TABLE `bi_pacs_servidor`
    ADD COLUMN `sync_auto_ativo`        TINYINT(1)   NOT NULL DEFAULT 0  COMMENT 'Se 1, aceita chamadas do cron externo (cron-job.org) para ping automático' AFTER `observacoes`,
    ADD COLUMN `sync_intervalo_minutos` INT          NOT NULL DEFAULT 60 COMMENT 'Intervalo entre pings automáticos, em minutos (1 a 1440)' AFTER `sync_auto_ativo`,
    ADD COLUMN `sync_cron_token`        VARCHAR(64)  NULL                COMMENT 'Token secreto usado pelo cron-job.org para autenticar a chamada' AFTER `sync_intervalo_minutos`,
    ADD COLUMN `sync_ultima_execucao`   DATETIME     NULL                COMMENT 'Data/hora da última execução automática recebida do cron externo' AFTER `sync_cron_token`;
